...
import { Sparkles, LogOut, Bell, Settings, UserCircle, Briefcase, FileText, Users, BarChart3, HelpCircle } from "lucide-react";
...
export function DashboardLayout({ children, userType, userName, userImage, sidebarItems }: DashboardLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-[#FFFBFC] font-sans flex text-slate-900">
      {/* Sidebar */}
      <aside className="w-72 bg-white border-r border-[#FDF0F4] fixed inset-y-0 left-0 flex flex-col z-20">
        <div className="h-36 flex items-center px-6 border-b border-[#FDF0F4]">
          ...
          <span className="ml-auto text-xs font-semibold px-2 py-1 bg-[#FEF7F9] rounded-md text-[#5B002C] uppercase tracking-wider">
            {userType}
          </span>
        </div>
      ...
      <main className="flex-1 ml-72 flex flex-col min-h-screen">
        {/* Header */}
        <header className="h-36 bg-white/80 backdrop-blur-md border-b border-[#FDF0F4] sticky top-0 z-10 px-8 flex items-center justify-between">
          <h1 className="text-xl font-bold text-[#3E0014]">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-3 pl-6 border-l border-[#FDF0F4]">
              <img src={userImage} alt={userName} className="w-10 h-10 rounded-full border-2 border-white shadow-sm object-cover" />
              <div className="hidden md:block">
                <p className="text-sm font-semibold text-[#3E0014]">{userName}</p>
